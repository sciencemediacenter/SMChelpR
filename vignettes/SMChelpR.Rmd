...
Viele Daten, die für die Data Reports verwendet werden, sind in der [SMC Data Collection](https://github.com/sciencemediacenter/DataCollection) verfügbar. Der Zugriff erfolgt über GraphQL. In R bieten wir zwei Wrapper für einen Datenzugriff über GraphQL an.
...
Außerdem erlaubt `GraphQL_get_table_vec` mittels `where`-Klauseln spezifischere Abfragen. Die Parameter `page_size` und `max_pages` können bei großen Datensätzen zur Pagination benutzt werden (setze `max_pages=Inf`) oder auch zum Abfragen eines Teildatensatzes dienen (z.B. auf GraphQL `limit=10` entspricht `max_pages=1` mit `page_size=10`).
```{r}
...
Der Aufruf von `SMC_theme_ggplot()` setzt viele Grafikparameter, aufbauende auf das `theme_minimal()` aus dem  `ggplot2` Paket. Darüber hinus wird insbesondere die Schriftart ["PlusJakartaSans"](https://fonts.google.com/specimen/Plus+Jakarta+Sans) und die Farbpalette eingesetzt, die im SMC standardmäßig für Grafiken genutzt wird (kann via `colors_SMC()` abgerufen werden).
...
Abbildung <-
  ...
  full_join(
    expand_grid(
      ...
      Monat = unique(storys_per_month$Monat)
    ),
    by = c("Monat", "type")
  ) |>
...
image_helper(
  ...
  plotly = TRUE,
  caption = paste(
    "Die Abbildung wurde mit plotly erstellt, sodass einige ggplot Features wie Untertitel oder Bildunterschriften nicht mehr verfügbar sind. Entpsrechend müssten entsprechende captions jetzt hier eingefügt werden."
  )
)
