...
Das Paket `SMChelpR` enthält verschiedene Funktionen, mit denen die [Data Reports des Science Media Center Germany](https://www.sciencemediacenter.de/alle-angebote/suchergebnis/?tx_solr%5Bfilter%5D%5B1%5D=type%3AData+Report) erstellt werden. Neben den Data Reports in html-Format stellen wir auch die Quarto-Dokumente (als Download oder alternativ auf Anfrage) zur Verfügung. Um die Quarto-Dokumente selbst rendern zu können, wird dieses R-Paket benötigt.
...
Die Hilfsfunktion `querystring_to_tabname_and_vec` erzeugt aus einfachen GraphQL-Abfragen (ohne Filter etc.) kopierbare Parametr für Variablen- und Tabellenname, die direkt in `GraphQL_get_table_vec` eingesetzt werden können.
...
Der Aufruf von `SMC_theme_ggplot()` setzt viele Grafikparameter, aufbauende auf das `theme_minimal()` aus dem  `ggplot2` Paket. Darüber hinus wird insbesondere die Schriftart "Aller" und die Farbpalette eingesetzt, die im SMC standardmäßig für Grafiken genutzt wird.
