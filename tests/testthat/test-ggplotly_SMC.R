...
test_that("ggplotly_SMC() decreases legend_y if x axis label is not given", {
  ...
  tmp_ggplot_with_mehrzeiliger_titel <-
    ...
    geom_point() +
    labs(
      title = "Beispiel-Plot im SMC-Theme - Schriftart: txyz \n test test test",
      x = ""
    ) +
    SMC_theme_ggplot()

  # create plotly objects
  tmp_ggplotly_with <- tmp_ggplot_with_xlabel |>
    ggplotly_SMC(x_axis_label_gegeben = TRUE, mehrzeiliger_titel = FALSE)
  tmp_ggplotly_without <- tmp_ggplot_without_xlabel |>
    ggplotly_SMC(x_axis_label_gegeben = FALSE, mehrzeiliger_titel = FALSE)
  tmp_ggplotly_with_and_mehrzeiliger_titel <- tmp_ggplot_with_mehrzeiliger_titel |>
    ggplotly_SMC(x_axis_label_gegeben = TRUE, mehrzeiliger_titel = TRUE)
  ...
  legend_y_with_and_mehrzeiliger_titel <- plotly_build(
    tmp_ggplotly_with_and_mehrzeiliger_titel
  )$x$layout$legend$y
...
test_that("ggplotly_SMC() attached font dependencies are attached if interactive == TRUE", {
  ...
  has_matching_element <- any(sapply(dependencies, function(element) {
    is.list(element) &&
      !is.null(element$src$href) &&
      grepl(
        "https://media.sciencemediacenter.de/static/fonts/circular",
        element$src$href
      ) &&
      !is.null(element$stylesheet) &&
      element$stylesheet == "web.css"
  }))

  # Expectation
  expect_true(
    has_matching_element,
    "The list does not contain an element with the specified href and stylesheet."
  )
})
