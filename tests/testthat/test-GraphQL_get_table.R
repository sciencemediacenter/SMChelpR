...
test_that("GraphQL_get_table_vec Data", {
  ...
  expect_equal(length(unique(story_testdaten_vec$title)), n)
  expect_equal(class(story_testdaten_vec), c("tbl_df", "tbl", "data.frame"))
})

test_that("GraphQL_get_table_vec Encoding", {
  expect_equal(story_testdaten_vec$ressort[1], "Energie & Mobilität")
  expect_equal(
    as.Date(story_testdaten_vec$publication_date[1]),
    as.Date("2022-03-17")
  )
})
...
test_that("GraphQL_get_table_vec Error handling", {
  # Test wrong URL - should error with HTTP 404
  expect_error(
    GraphQL_get_table_vec(
      tabellenname = "test_R_Packages_test_story",
  ...
  # Test non-existent field - should error with validation message
  expect_error(
    GraphQL_get_table_vec(
      tabellenname = "test_R_Packages_test_story",
    ...
    "field 'story_no2' not found"
  )

  # Test non-existent table - should error with validation message
  expect_error(
    GraphQL_get_table_vec(
...
test_that("GraphQL_get_table_string Data", {
  ...
  expect_equal(length(unique(story_testdaten_string$title)), n)
  expect_equal(class(story_testdaten_string), c("tbl_df", "tbl", "data.frame"))
})

test_that("GraphQL_get_table_string Encoding", {
  expect_equal(story_testdaten_string$ressort[1], "Energie & Mobilität")
  expect_equal(
    as.Date(story_testdaten_string$publication_date[1]),
    as.Date("2022-03-17")
  )
})
