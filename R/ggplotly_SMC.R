...
ggplotly_SMC <- function(
  ...,
  legende_unten = TRUE,
  x_axis_label_gegeben = TRUE,
  mehrzeiliger_titel = FALSE,
  interaktiv = FALSE,
  ggplotly_params = list()
) {
  ...
  if (legende_unten) {
    abbildung <- abbildung |>
      layout(
        legend = list(
          orientation = "h",
          entrywidth = get_param(ggplotly_params, "legend_entrywidth", Inf),
          yanchor = "bottom",
          y = legend_y,
          xanchor = "center",
  ...
  abbildung <- config(
    abbildung,
    modeBarButtonsToRemove = get_param(
      ggplotly_params,
    ...
    abbildung$dependencies <- c(
      ...
      list(
        htmltools::htmlDependency(
          ...
          version = "1.0",
          src = c(
            href = "https://media.sciencemediacenter.de/static/fonts/circular"
          ),
          stylesheet = "web.css"
        )
      )
